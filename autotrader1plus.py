...
class AutoTrader(BaseAutoTrader):
    ...
    def __init__(self, loop: asyncio.AbstractEventLoop, team_name: str, secret: str):
        ...
        self.is_buyer_market_static = [-1,-1]
    ...
    def on_order_book_update_message(self, instrument: int, sequence_number: int, ask_prices: List[int],
        ...
        if instrument == Instrument.FUTURE:
            ...
            if self.round_count == 100:
                ...
                is_buyer_market = 1 if buy_total_cap > sell_total_cap*buyer_seller_margin else -1
                is_seller_market = 1 if buy_total_cap*buyer_seller_margin < sell_total_cap else -1
                if self.is_buyer_market_static[0] != -1 and self.is_buyer_market_static[1] != -1:
                    capital_dicrection_changed = self.is_buyer_market_static[1] + self.is_buyer_market_static[0] == 1
                else:
                    capital_dicrection_changed = True
                ...
                should_buy = self.bid_id == 0 and new_bid_price != 0 and self.position < POSITION_LIMIT and capital_dicrection_changed
                if is_buyer_market == self.is_buyer_market_static[1] and should_buy:
                ...
                should_sell = self.ask_id == 0 and new_ask_price != 0 and self.position > -POSITION_LIMIT and capital_dicrection_changed
                if is_seller_market and self.is_buyer_market_static[1] ==0 and should_sell:
                ...
                is_buyer_market_store = -1 if is_buyer_market == is_seller_market else is_buyer_market == 1
                self.is_buyer_market_static[0],self.is_buyer_market_static[1] = self.is_buyer_market_static[1],is_buyer_market_store
    ...
    def execute_order_by_fut(self, bid_prices, ask_prices):
        ...
        new_bid_price = bid_prices[0] + price_adjustment if bid_prices[0] != 0 else 0
        new_ask_price = ask_prices[0] + price_adjustment if ask_prices[0] != 0 else 0
        self._execute_order(new_bid_price, new_ask_price)
    ...
    def _execute_order(self, new_bid_price, new_ask_price):
        buy_total_cap, sell_total_cap = self.calculate_sides_capital()
    ...
    def execute_in_parallel(self, func, *args):
        self.thread_pool.submit(func, self, args)
    ...
    def on_order_filled_message(self, client_order_id: int, price: int, volume: int) -> None:
        ...
        if client_order_id in self.bids:
            self.position += volume
            order_id = next(self.order_ids)
            self.send_hedge_order(order_id, Side.ASK, MIN_BID_NEAREST_TICK, volume)
